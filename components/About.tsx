...
const About: React.FC = () => {
    ...
    useEffect(() => {
        if (!isEditing) {
            setEditValues({
                ...
                description: config.about_description || "ATELIER no es solo una marca de iluminación; es un laboratorio de ingeniería lumínica donde el futuro de la arquitectura se encuentra con la precisión técnica.",
  ...
  return (
    <div id="about" className="bg-black pt-32 pb-24 text-white relative group/about">
         {/* Admin Controls */}
      {isAdmin && (
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-[8rem] font-thin tracking-tighter leading-none mb-12">
            {isEditing ? (
                 ...
                 <span dangerouslySetInnerHTML={{ __html: config.about_headline?.replace(".", "<span class='italic opacity-30'>.</span>") || "LEGADO <span class='italic opacity-30'>VIRTUAL.</span>" }} />
            )}
          </h1>
          {isEditing ? (
             <textarea 
      ...
      <section className="px-6 relative">
        ...
        <div className="max-w-7xl mx-auto space-y-32">
          {(isEditing ? editValues.items : (config.about_history || [])).map((item: any, index: number) => (
            <div
              key={index}
