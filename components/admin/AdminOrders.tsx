  ...
  const handleSave = async (order: Partial<Order>) => {
    try {
        if (order.id && !order.id.startsWith("NEW_")) {
           ...
           setOrders(prev => prev.map(o => o.id === order.id ? { ...o, ...order } as Order : o));
        } else {
                ...
                [...Array(3)].map((_, i) => <TableRowSkeleton key={i} />)
                 ...
                 <tr>
                    <td colSpan={6} className="py-24 text-center opacity-30">
                        <p className="font-futuristic text-[10px] tracking-widest uppercase">Sin Órdenes</p>
                    </td>
                 </tr>
             ) : (
                 filteredOrders.map((order) => {
                     ...
                     return (
                        <tr key={order.id} className="group hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                            ...
                            <td className="px-6 py-4 font-light">${total.toLocaleString()}</td>
                            <td className="px-6 py-4">
                                <span 
                                    className={`px-2 py-1 text-[9px] font-futuristic uppercase tracking-wider rounded select-none border ${
                                        order.status === "delivered" ? "border-green-500/30 bg-green-500/10 text-green-600" :
                                        order.status === "cancelled" ? "border-red-500/30 bg-red-500/10 text-red-600" :
                                        order.status === "shipped" ? "border-blue-500/30 bg-blue-500/10 text-blue-600" :
...
const OrderForm: React.FC<OrderFormProps> = ({ order, availableProducts, onClose, onSave }) => {
    ...
    const addItem = (product: Product) => {
        setFormData(prev => {
            ...
            if (existing) {
                return {
                    ...prev,
                    items: currentItems.map(i => i.product.id === product.id 
    ...
    const updateItemQuantity = (productId: string, qty: number) => {
        setFormData(prev => ({
            ...prev,
            items: prev.items?.map(i => i.product.id === productId ? { ...i, quantity: Math.max(1, qty) } : i) || []
        }));
    };
    ...
    const updateItemNote = (productId: string, note: string) => {
        setFormData(prev => ({
            ...prev,
            items: prev.items?.map(i => i.product.id === productId ? { ...i, note } : i) || []
        }));
    };

    const total = formData.items?.reduce((sum, item) => sum + (item.price * item.quantity), 0) || 0;
                ...
                <div className="flex-1 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <label className="text-[10px] font-futuristic tracking-widest text-neutral-500 uppercase">Nombre</label>
                            <input 
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-futuristic tracking-widest text-neutral-500 uppercase">Apellido</label>
                            <input 
                                 className="w-full bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-white/10 p-3 text-sm focus:border-black dark:focus:border-white outline-none"
                        ...
                        <div className="col-span-2 space-y-2">
                            <label className="text-[10px] font-futuristic tracking-widest text-neutral-500 uppercase">Email</label>
                            <input 
                                 className="w-full bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-white/10 p-3 text-sm focus:border-black dark:focus:border-white outline-none"
                         ...
                         <div className="col-span-2 space-y-2">
                            <label className="text-[10px] font-futuristic tracking-widest text-neutral-500 uppercase">Dirección</label>
                            <input 
                                 className="w-full bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-white/10 p-3 text-sm focus:border-black dark:focus:border-white outline-none"
                         ...
                         <div className="space-y-2">
                            <label className="text-[10px] font-futuristic tracking-widest text-neutral-500 uppercase">Ciudad</label>
                            <input 
                                 className="w-full bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-white/10 p-3 text-sm focus:border-black dark:focus:border-white outline-none"
                                value={formData.city}
                ...
                <div className="flex-1 space-y-6 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto border border-neutral-100 dark:border-neutral-800 rounded p-2 space-y-2 min-h-[200px]">
                        {formData.items?.map((item, idx) => (
                            <div key={item.product.id} className="bg-neutral-50 dark:bg-neutral-900 p-3 rounded flex gap-3 items-start group">
                                ...
                                <div className="flex-1 space-y-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <label className="text-[9px] uppercase tracking-wider text-neutral-400">Qty:</label>
                                        <input 
                                            type="number"
                                            ...
                                            min="1"
                                        />
                                        <label className="text-[9px] uppercase tracking-wider text-neutral-400 ml-2">Nota:</label>
                                        <input 
                                            type="text"
                    ...
                    <div className="flex justify-between items-end pt-4 border-t border-neutral-200 dark:border-neutral-800">
                         <div className="text-right w-full">
                            <span className="text-[10px] font-futuristic tracking-widest uppercase mr-4">Total Estimado</span>
